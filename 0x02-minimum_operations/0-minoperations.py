...
def minOperations(n: int) -> int:
    """
    sdvmsmodfkvmsd  sdfofvns sdsvsoidn fdv
    """
    # BFS approach
    if n <= 1:
        return 0
    ...
    queue = deque(
        [(1, 0, 0)])  # (current_length_of_buffer, clipboard_content, steps)
    ...
    while queue:
        ...
        if clipboard > 0 and buffer_len + clipboard <= n and (
                buffer_len + clipboard, clipboard) not in visited:
